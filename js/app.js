...
$('#container').attr('current-season', getCurrentSeaoson);
...
function getFoodHtml(id, cat, icon, name, rating, button = 'edit') {
	...
	if (button == "edit") {
		...
		food += '<div class="food-skip"><span class="material-symbols-rounded">cached</span></div>';
	}
...
function updateCalenderList() {
	...
	for (let i = 0; i < aryDates.length; i++) {
		var weekArray = aryDates[i].split('.'),
			...
			skipButton = (today) ? "skip" : "",
			previousDays = (i < 3) ? " prev" : "",
			nextDays = (i > 3) ? " next" : "",
			addFood = (today) ? '<div id="add_new_food_cal" class="plus-icon"><span class="material-symbols-rounded">add</span></div>' : '';

		var foodByDate = $('<div data-weekday="' + weekday + '" class="food-by-date' + today + previousDays + nextDays + '">' + addFood + '<div class="selected-date"></div><div class="food-list empty"><img src="images/shine-young-woman-meditating-in-nature-surrounded-by-butterflies-1.png"><div class="selected">Preskočen ručak na ovaj dan.</div></div></div>').appendTo(foodWrapper),
		...
		if (separated.length) {
			...
			foodListDate.html('').removeClass('empty');
			...
			foodListDate.removeClass('empty').html('').append(getFoodHtml(food.id, food.cat, food.icon, food.name, food.rating, skipButton));
...
function updateCalenderListSingleDay(foodByDate) {
	...
	for (let i = 0; i < aryDates.length; i++) {
		...
		if (separated.length == 0 && (nextDays || today)) {
			...
			foodListDate.removeClass('empty').html('').append(getFoodHtml(food.id, food.cat, food.icon, food.name, food.rating, ""));
		}
	}
	if (needsUpdate) updatePantryMain();
	return needsUpdate;
...
function updateFoodList(json) {
	...
	for (let i = 0; i < json['foods'].length; i++) {
		...
		foodList.append(getFoodHtml(currentFood.id, currentFood.cat, currentFood.icon, currentFood.name, currentFood.rating, "edit"));
	}
}
...
function updateCalendarFoodList(currentFood) {
	...
	$('.food[food-id="' + currentFood.id + '"]').each(function () {
		var editing = ($(this).closest('.section[data-cat="foods"]').length) ? "edit" : '';
...
$('body').on('click', '.date:not(.active)', function () {
	...
	if (iconCont.find('.icon.active').length < 1) {
		iconCont.addClass("error");
		error = true;
	}
	...
	$('#food-editor').fadeOut();
	$('.section.floating').fadeOut(300, function () {
		$(this).removeClass('floating').removeAttr('style');
	});
	...
	// Are you sure skip
}).on('click', '.food-skip', function () {
	$('#skip-you-sure img').remove();

	$('#skip-you-sure .sure-inner').prepend('<img src="' + $(this).closest('.food').find('img').attr('src') + '"/>');

	$('#skip-you-sure .delete').attr('food-id', $(this).closest('[food-id]').attr('food-id'));
	$('#skip-you-sure').fadeIn(300);
	...
	// Skip you sure
}).on('click', '#skip-you-sure button.delete', function () {

	let foodID = $(this).attr('food-id'),
		food = $('.section[data-cat="calender"] .today .food[food-id="' + foodID + '"]'),
		foodList = food.closest('.food-list'),
		currentFood = json["foods"].filter(obj => {
			return obj.id === foodID;
		}),
		needsUpdate = false;
	...
	for (let i = 0; i < currentFood.length; i++) {
		var index = (currentFood.length) ? json["foods"].indexOf(currentFood[i]) : "no";
		...
		json['foods'][index].last = json['foods'][index].prev;
	}
	...
	if (foodList.find('.food').length == 0) {
		foodList.addClass('empty').html('<img src="images/shine-young-woman-meditating-in-nature-surrounded-by-butterflies-1.png"><div class="selected">Preskočen ručak na ovaj dan.</div>');
	}


	$('#skip-you-sure').fadeOut(300);

	// Are you sure cancel
}).on('click', '#skip-you-sure button.save', function () {
	$('#skip-you-sure').fadeOut(300);

	// Add new food to calender
}).on('click', '#add_new_food_cal', function () {

	$('.section[data-cat="foods"]').addClass('floating').fadeIn(300);

}).on('click', '.section.floating[data-cat="foods"] .food', function () {
	var foodID = $(this).attr('food-id'),
		currentFood = json["foods"].filter(obj => {
			return obj.id === foodID;
		});
	...
	if (currentFood.length) {
		...
		var today = new Date(),
			currentDay = DayAsString(today.getDay()) + '.' + today.getDate() + '.' + months[today.getMonth()] + '.' + today.getFullYear();


		if (json["foods"][index]) {

			json["foods"][index].last = currentDay;

			var food = json["foods"][index],
				foodList = $('.food-by-date.active .food-list').removeClass('empty'),
				html = getFoodHtml(food.id, food.cat, food.icon, food.name, food.rating, 'skip');


			if (foodList.find('.food').length === 0) {
				foodList.html(html);
			} else {
				foodList.prepend(html);
			}
